...
// Blog post data — retained as static default; replace with API call when CMS endpoints are built
interface BlogPost {
  id: string;
  title: string;
  titleThai: string;
  ...
  const renderContent = (text: string) => {
    return text.split('\n\n').map((block, i) => {
      ...
      if (block.startsWith('**') && block.endsWith('**')) {
        return (
          <h3 key={i} className="mb-3 mt-6 text-lg font-bold text-slate-900 dark:text-slate-100">
            {block.replace(/\*\*/g, '')}
          </h3>
        );
      }
    ...
    <PublicLayout>
      ...
      <article className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="mb-8 flex flex-wrap items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
          ...
          <button
            onClick={handleShare}
            className="relative ml-auto flex cursor-pointer items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm transition-colors hover:bg-slate-100 dark:hover:bg-slate-800"
          >

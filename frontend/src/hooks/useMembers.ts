...
export function useMembers(options: UseMembersOptions = {}): UseMembersReturn {
  ...
  const fetchMembers = useCallback(async () => {
    try {
      ...
      const errorMessage = getErrorMessage(err, 'Failed to fetch members');
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      try {
        ...
        const errorMessage = getErrorMessage(err, 'Failed to search members');
        setError(errorMessage);
      } finally {
        setLoading(false);
      }
...
export function useMemberProfile(options: UseMemberProfileOptions): UseMemberProfileReturn {
  ...
  const fetchMember = useCallback(async () => {
    try {
      ...
      const errorMessage = getErrorMessage(err, 'Failed to fetch member profile');
      setError(errorMessage);
    } finally {
      setLoading(false);
    }
  ...
  const updatePrivacySettings = useCallback(
    async (settings: { showPhone?: boolean; showEmail?: boolean; showAddress?: boolean }) => {
      try {
        ...
        const errorMessage = getErrorMessage(err, 'Failed to update privacy settings');
        setError(errorMessage);
        throw err;
      } finally {
        setLoading(false);

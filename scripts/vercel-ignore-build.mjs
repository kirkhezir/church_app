...
function shouldTriggerFrontendBuild(changedFiles) {
  return changedFiles.some((file) => frontendOrSharedPatterns.some((pattern) => pattern.test(file)));
}
...
try {
  const diffOutput = execSync(`git diff --name-only ${previousSha} ${currentSha}`, {
    encoding: 'utf8',
  ...
  console.log('Only backend/non-frontend files changed, skipping frontend build.');
  process.exit(0);
} catch (error) {
  console.log('Unable to compute changed files, running frontend build by default.');
